...
> ⚠️ **MIGRATION IN PROGRESS -- one wrapper is not a pointer yet.**
> `.windsurf/workflows/start-sdlc-feature.md` is a thin pointer at this file.
> **`.claude/commands/start-sdlc-feature.md` is still the old full 319-line copy**, so the Claude
> Code lane is executing the PRE-FIX procedure:
>
> 1. **The approve+automerge grant there is still UNBOUNDED** -- it says to arm
>    merge-when-pipeline-succeeds "as soon as the MR opens" with no read-the-diff-first, no scope,
>    no review date, and without the "a team that never recorded one does not have it" clause. This
>    is the item on this list with a security consequence rather than a correctness one, so it is
>    first.
...
**A `TODO` value is the same as a missing file -- STOP and ASK for that field too.** This is not a
hypothetical: `/convert-repo-to-agentic` Step 10.8 writes every field it found no evidence for as a
literal `TODO:` line on purpose (a TODO is honest; a guessed epic pollutes another team's velocity),
so a freshly converted repo routinely has a `BINDING.yml` that exists and is still unusable. Check
the values, not just the file:

```bash
# Exits 1 and names the offending field(s) if any binding value is absent or still TODO.
# Reference implementation: scripts/fleet/project_verify.py binding_gaps() -- same rule,
# and the two must agree. This is the shape that works: CAPTURE the value, then test it.
python3 - "$REPO/BINDING.yml" <<'PY'
import re, sys
FIELDS = ("jira_project", "epic", "board", "dev_classification", "assignee_account_id")
text = open(sys.argv[1], encoding="utf-8", errors="replace").read()
# [^\S\n] is same-line whitespace. Plain \s* would match the NEWLINE too, so an empty
# `epic:` would absorb the next field's value and read as filled.
# Do NOT rewrite this as a lookahead ((?!['\"]?TODO\b) etc.): a zero-width assertion
# after a variable-width class is defeated by backtracking. The engine retries the
# class one position to the left, the assertion is then evaluated at the SPACE rather
# than at the value, and `epic: TODO` matches -- i.e. reads as filled. Measured: the
# lookahead form caught `epic:TODO` and `epic:` but passed `epic: TODO` and
# `epic: "TODO"`, which are the two ways it is actually written.
# re.M only -- NOT re.I. YAML keys are case-sensitive, so `EPIC:` is not a binding any
# consumer can read and has to count as absent, which is what binding_gaps() does. An
# re.I here made this gate the more permissive of the two in the one direction that
# matters: `EPIC: ABC-1` printed BINDING OK while R7 failed the same file. Case
# insensitivity belongs on the VALUE test below, where `todo` and `TODO` really are one
# sentinel.
S = r"[^\S\n]*"
bad = []
for f in FIELDS:
    m = re.search(r"^%s%s%s:(.*)$" % (S, f, S), text, re.M)
    value = m.group(1).strip() if m else ""
    if not value or re.match(r"^['\"]?TODO\b", value, re.I):
        bad.append(f)
print("BINDING INCOMPLETE -- resolve before creating a ticket: %s" % ", ".join(bad)) if bad \
    else print("BINDING OK")
sys.exit(1 if bad else 0)
PY
```

The same check is enforced mechanically at the project layer by
`scripts/fleet/project_verify.py --require-binding` (R7).
...
`/convert-repo-to-agentic` creates `BINDING.yml` for newly converted repos, so a *missing* file
should only ever appear on a repo that predates it -- an incomplete one appears whenever the
conversion found no tracker evidence.
...
The preflight script lives in the Agentic-Repos framework checkout, not in the target repo --
...
**AMBIGUOUS** -- two live projects share that exact name. Re-run with the full path; do not let
anything pick one for you.

**FAIL** -- read the reason. The script gates on `glab api user` first, so `not authenticated` means
the token, never the repo. For any other FAIL, confirm liveness directly before concluding the repo
is dead, and say you verified by the alternate route rather than that preflight passed:
