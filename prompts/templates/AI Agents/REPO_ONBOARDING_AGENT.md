...
- GitLab URL: `git@gitlab.com:your-org/GROUP/repo.git`
...
`main` (caught 2026-08-05 converting `PRZO/fx`, whose default branch is `master` — every
...
BOT_FILTER="svc-automation|renovate|renovate-bot|dependabot|ci-user|gitlab-ci-token"
...
**Generated by:** Repo Onboarding Agent
...
# the adapter log, on the console, and in the Step 6 completion report, with the
# reason. Kill switch: GRAPHIFY_ADAPTER=0 remains the operator's explicit opt-out
# (removal drill: set it and this block is a no-op that writes nothing).
...
case "$GRAPHIFY_FLAG" in
  1|true|yes|on)  # mirror the adapter's fail-closed kill switch: only an explicit
                  # affirmative (or unset) runs; any other value writes NOTHING —
                  # not even the log file.
    ...
    GRAPHIFY_PY="$(bash "$FRAMEWORK_HOME/scripts/onboarding/ensure_graphify.sh" \
                     2> "${REPO_PATH}/Generated/Analysis/GRAPHIFY_BOOTSTRAP.err")" || _graphify_rc=$?
    if [ "$_graphify_rc" -eq 0 ] && [ -n "$GRAPHIFY_PY" ]; then
      rm -f "${REPO_PATH}/Generated/Analysis/GRAPHIFY_BOOTSTRAP.err"
      ...
      # adapter log, (2) the console, and (3) Step 6's completion report reads
      # this marker for its "Dependency Graph" line.
      _graphify_reason="$(cat "${REPO_PATH}/Generated/Analysis/GRAPHIFY_BOOTSTRAP.err" 2>/dev/null | tr '\n' ' ')"
      {
        echo "GRAPHIFY_SKIPPED rc=${_graphify_rc} reason: ${_graphify_reason:-unknown}"
...
lines = [
    ...
    "",
    f"**Generated by:** Repo Onboarding Agent",
...
**Analysis Date:** $TODAY
**Analyzed By:** Repo Onboarding Agent
...
**Derivation date:** $TODAY
**Source:** Automated analysis of implementation (Repo Onboarding Agent)
...
> Bot filter applied: renovate, renovate-bot, dependabot, ci-user, gitlab-ci-token
...
**Step 10.8: Create BINDING.yml (project tracker binding)**
...
jira_project: <TRACKER KEY> # e.g. BILLING
...
- **`jira_project` (and `board`, when the annotation carries it)** — take `$JIRA_PROJECT` from
  ...
  - a bare key — `BILLING` → `jira_project: BILLING` (this rule yields no `board`).
  - a full board URL — `https://<jira-host>/jira/software/c/projects/BILLING/boards/4321` →
    parse `/projects/<KEY>/boards/<ID>` → `jira_project: BILLING` **and** `board: 4321`.
...
origin` — and reduce it to a group path: drop the scheme/user and host (`git@<host>:` or
     ...
     `git@gitlab.com:your-org/apps/TOOLS/agentic-repo.git` reduces to
     `your-org/apps/TOOLS`.
...
- `BILLING` and `4321` are this template's illustrative values, not defaults. Write them, or any
  other value, ONLY when one of the cited sources above produced them for _this_ repo — a repo
  whose own git history is `BILLING-…` resolves `jira_project: BILLING` legitimately — and name
  that source in the report. Never write any value as a default, a fallback, or a filled-in
  example. The invariant is "no unevidenced value", not "no particular value".
...
| ------------------- | ------------- | ---------------------------------------------------------------------------------------------------------- | ------- |
| jira_project        | BILLING       | PHASE1_DETECTION.md § Variable Assignments → JIRA_PROJECT (catalog-info.yaml annotations.jira/project-key) | DERIVED |
| board               | 4321          | same annotation — parsed from `/projects/BILLING/boards/4321`                                              | DERIVED |
...
matched, e.g. `sibling checkout billing-ws, origin group your-org/apps/BILLING, matches
...
consistently here (caught during a real conversion). The wrapper
...
2. **A sibling-directory walk — best-effort, layout-dependent, not a guarantee.** It checks
   ...
   naming the env var, not silently misbehave — verified empirically (2026-08-03, during a
   real conversion): with a non-sibling layout and no env var set, the script prints
...
# canonical scripts/onboarding/verify_citations.sh (T3 hard gate).
#
...
#   in the fixture repo to be flagged as non-resolving (known semantic mismatch).
#   Run with: bash Generated/scripts/run_verify_citations.sh \
#               --repo-path /path/to/fixture-repo
...
# AC22: verify_citations.sh now defaults VALIDATION_SUMMARY.md to
...
# contaminates a read-only fixture directory such as a shared fixture checkout.
bash Generated/scripts/run_verify_citations.sh Knowledge/CODE_INDEX.md \
  --summary-path Generated/VALIDATION_SUMMARY.md
# AC7 regression: verify fixture-repo citations at a pinned SHA
...
HEAD_SHA=$(git -C /path/to/fixture-repo rev-parse HEAD)
...
| Budget limit                                    | ~360,000 chars | ~90,000 tokens | framework history |
...
| Knowledge/golden/GOLDEN_FACTS.jsonl + .md                   | [created/asserted/unchanged]             | Step 15.7 eval gate; COMMITTED (drift anchors travel with the repo), derive-once on re-runs    |
...
echo "=== (f) BINDING.yml exists and carries an evidenced tracker binding ==="
...
if [[ -f "$REPO_PATH/BINDING.yml" ]]; then
  ...
  # The template's illustrative values are not forbidden, they are only allowed WITH
  # evidence (a repo whose git history is BILLING-… resolves jira_project: BILLING
  # legitimately). Flag them so the per-field report is checked, never auto-fail.
  if grep -qE 'BILLING|4321' "$REPO_PATH/BINDING.yml"; then
    echo "WARN: BINDING.yml contains BILLING or 4321 — these are the template's illustrative values; confirm the Step 10.8 per-field report"
    echo "      names the cited source that produced them for THIS repo; if not, they leaked."
  else
    echo "PASS: BINDING.yml carries no unevidenced illustrative defaults"
  fi
else
...
python3 "$FRAMEWORK_HOME/scripts/onboarding/golden_facts.py" derive "$REPO_PATH"
...
# or the loud-skip GRAPHIFY_BOOTSTRAP.err), CODE_INDEX.md is registered in
...
Golden Facts: [N/N hold, from Step 15.7] — see Knowledge/golden/GOLDEN_FACTS.md
...
- [ ] golden_facts.py derive + assert both rc 0 (Step 15.7); `Knowledge/golden/GOLDEN_FACTS.{jsonl,md}` present and the fact count reported in the Step 16 summary
